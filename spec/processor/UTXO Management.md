...
UTXO-based chains have practical requirements for efficient operation that can
...
When `sriXYZ` is burned, Serai creates an output for `XYZ`, incurring a fee in the 
containing transaction. In response, Serai linearly amortizes this fee over all outputs that
transaction creates.

Although Serai could charge a fee in advance, either static or dynamic to views of
the fee market, it would risk the fee being inaccurate. If it's too high, users have
paid fees they shouldn't have; if it's too low, Serai is insolvent. Therefore,
the actual fee is amortized, rather than an estimation being prepaid.

Serai could report a view, and when burning occurs, that view could be locked
in as the basis for transaction fees used to fulfill the output.
This would require burns specify the most recent fee market view they're aware
of, signifying their agreement, with Serai erroring if a new view is published
before the burn is included on-chain. Not only would this require more data be
published to Serai (widening data pipeline requirements), it would prevent any
...
may not allow spending outputs from transactions that have yet to be ordered.
For its own safety, Monero only allows spending outputs from transactions that have 10 confirmations.
...
this is the highest speed at which outputs may be processed.
...
The solution implemented in Serai is to consume all burns from the start of a
global queue that can be satisfied under currently available inputs. Although the
consumed queue may have 256 items—more than an external network with an
`output_creation_rate` of 16 can process in a single tick—Serai can immediately
set a finite bound on execution duration.

For the above example parameters, Serai would create 16 outputs within its tick,
ignoring the necessity of a change output. These outputs would _not_ correspond to
outputs expected in response to burns, but instead create
16 "branch" outputs. One tick later, when the branch outputs are available to
spend, each would fund the creation of 16 expected outputs.
...
require a sliding fee on Serai's side, as it does not need to limit the on-chain
rate of burns, which means it does not adversely affect normal users. While
...
spendable at a given moment, and spending it will prevent its availability for one
tick of a newly started clock.
...
Within a tick of operations, the full balance of Serai can be
...
If a full tick elapses, due to the front of the global queue having a
notably large burn, then the entire global queue will be consumed, as full input
availability allows all potential burns to be satisfied in a solvent
system.
...
created. This TX incurs fees but has no burn-related outputs over which to amortize
them, thereby accumulating operating costs.
...
Serai accrues operating costs and tracks them with each created
transaction. When a created transaction has
...
an aggregation transaction. Serai would pay for the transaction fee, which would
go to the malicious miner.
...
from other users, from which they profit.
...
assumed infeasible to perform at scale. Even if demonstrated feasible, such an attack
would not present a critical vulnerability for Serai, only a low/medium/high
vulnerability for its users (though one it would still be our responsibility
to rectify).
