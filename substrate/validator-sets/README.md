...
the key using only their existing keys (as a newly created key cannot be used
...
With the publication of the oraclization key, necessary to interact with
...
The validator set for the Serai network itself does not need an external key and does not
...
set for an external network is decided, the newly declared validator set is now able to
publish their keys onto Serai. Eventually, the new validator set is expected to accept a
...
the new validator set's declared external key), which declares the newly declared set as the current
set.
...
Auxiliary keys are signed by the validator that uses them, by virtue of them
...
for which Substrate traditionally uses a host function for signing (because performing
cryptography within the runtime is sufficiently slow, native code is generally preferable
instead). As these proofs are simple (they're Schnorr
...
"Session Keys" traditionally do within a Substrate runtime. One distinction
is that Session Keys aim to declare a key per purpose, while Serai only
...
scheme to produce subkeys for each intended usage, as enabled by our choice of
...
allocating stake to a validator that isn't one's self.

If a validator allocates stake that is indivisible by the amount required for
a key share, the validator will be considered for an amount of key shares corresponding
...
In both cases, the selected validator's stake will be entirely considered for
...
oraclization key. Due to the lack of explicit on-chain validated evidence, 
...
A `PreInherents` hook is defined that _must_ be integrated into the runtime
