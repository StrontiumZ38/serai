...
Serai is a cross-chain decentralized exchange and requires decentralized exchange
functionality to operate. This pallet provides that functionality for the Serai protocol.
...
rather the MIT-licensed [`serai_primitives`] library. This is intentional so that
applications may integrate the underlying formulas, enabling the calculation of quotes,
without being subject to this crate's AGPL 3.0 license.
...
for transfers. However, the interface for these tokens ([`serai_abi::dex::Call`],
[`serai_abi::dex::Event`]) is entirely implemented within this crate.
